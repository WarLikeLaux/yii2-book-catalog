...
final readonly class PipelineFactory
{
    ...
    public function createWithFileLifecycle(): PipelineInterface
    {
        return (new Pipeline())
            ->pipe(new TracingMiddleware($this->tracer))
            ->pipe(new IdempotencyMiddleware($this->idempotencyService))
            ->pipe(new FileLifecycleMiddleware($this->fileStorage))
            ->pipe(new TransactionMiddleware($this->transaction));
    }
    ...
    public function createWithFileLifecycleWithoutIdempotency(): PipelineInterface
    {
        return (new Pipeline())
            ->pipe(new TracingMiddleware($this->tracer))
            ->pipe(new FileLifecycleMiddleware($this->fileStorage))
