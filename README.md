...
#### 13.4. UseCaseExecutor использует Yii::t() для переводов

**Компромисс:** `UseCaseExecutor` использует `Yii::t()` для переводов сообщений об успехе/ошибках, создавая зависимость от Yii2 в application layer.

**Почему:** Переводы сообщений — это cross-cutting concern, который логично обрабатывать в `UseCaseExecutor`. Использование `Yii::t()` — стандартный подход в Yii2, который обеспечивает интеграцию с системой переводов фреймворка.

**Что получили:**
* Централизованная обработка ошибок и уведомлений
* Интеграция с системой переводов Yii2
* Минимальная зависимость от фреймворка (только для переводов)
* Возможность легко заменить на другой механизм переводов через порт

#### 13.5. Итоговый баланс

Все компромиссы приняты осознанно и документированы. Результат:
* ✅ Application layer независим от фреймворка (кроме переводов)
