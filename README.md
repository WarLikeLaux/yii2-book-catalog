...
Этот проект — практическая реализация каталога книг на базе **Yii2 Basic** и **PHP 8.4**. Используется **Clean-ish** архитектура, демонстрирующая возможности написания масштабируемого кода на классическом фреймворке.
...
Чтение и запись разделены (паттерн **CQS**), все внешние зависимости вынесены в порты:
*   **Запись (команды):** любое изменение в системе (создание книги, подписка) — это отдельный **Use Case**. Данные поступают через строго типизированные **Command DTO**. Это гарантирует отсутствие «каши» из параметров.
*   **Чтение (запросы):** чтобы не тянуть тяжелые модели там, где нужно просто показать список, используются Query-порты. Они возвращают легкие DTO, что экономит память и время.
...
*   **Формы:** ActiveRecord не используется напрямую в формах. Для каждой задачи (создание книги, фильтрация) реализована своя форма, проверяющая данные.
...
*   **Трассировка (Distributed Tracing):** видно всё, что происходит «под капотом». Каждый запрос в базу или задачу в очереди можно изучить в Buggregator: затраченное время, параметры и место возникновения задержки.
...
*   **Rector:** автоматически обновляет код до PHP 8.4 и исправляет старые подходы на современные.
...
*   **UX:** поиск на главной странице обернут в **PJAX** для фильтрации без перезагрузки.
...
*   **Решение:** созданы свои DTO для страниц. Репозиторий делает выборку и возвращает «чистый» результат.
*   **Адаптер:** только в самом конце, перед выводом на экран, специальный адаптер превращает DTO обратно в формат, понятный виджетам Yii2.
...
*   **Реализации:** в инфраструктуре определяется: «вот как именно отправляется SMS через конкретный шлюз».
...
Внедрена система трассировки, чтобы не гадать, почему запрос тормозит или что именно пошло не так в очереди.

#### 🏗 Как это работает?
Нет необходимости вручную прокидывать логи по всему коду. Используются **декораторы**: они оборачивают репозитории и сервисы, автоматически замеряя время и записывая детали (например, SQL-запросы).
