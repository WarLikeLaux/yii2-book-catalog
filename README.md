...
<p align="center">
  <b>🏛 Clean-ish Architecture</b> • <b>⚡ CQS Pattern</b> • <b>🎯 Value Objects</b> • <b>📨 Domain Events</b> • <b>🔄 Async Fan-out</b> • <b>🤖 AI Governance</b>
</p>
...
> **🤯 Зачем так сложно?**
>
> Этот проект — мой **Architectural Showcase**. Я хотел доказать, что **Yii2** — это не только про "быстрый старт", но и надежный фундамент для сложных систем, если приложить к нему **Clean Architecture**, **DDD** и **SOLID**.
>
> Здесь всё "по-взрослому": **Deptrac** следит за слоями, **Infection** мучает тесты, а **PHPStan** (Level 9) не прощает ошибок.
>
> **🦖 Зачем взял этого динозавра (Yii2) с кучей магии?**
>
> Многие хоронят Yii2 (да и сам PHP), но я показываю симбиоз поколений: стабильное ядро + **PHP 8.4** (Enums, Readonly, Attributes).
> Я изгнал "магию" сервис-локатора (`Yii::$app->service`): здесь правит балом **Dependency Injection**. Код честный: если классу что-то нужно, он заявляет об этом в конструкторе.
>
> **🧠 Это очередной "нейрослоп" от ChatGPT?**
>
> Категорически нет. Вы всё ещё копипастите код из веб-чатов?
> Разработка уже давно вышла на новый уровень благодаря автономным терминальным агентам. Я использую их как экзоскелет, а не как замену мозгам. Весь код написан по моим строгим контрактам, прошел моё Code Review и тесты. Я показываю, как **управлять** нейросетями (Senior Experience + AI), чтобы строить крутую архитектуру, а не генерировать мусор.
>
> **🤔 А есть подвох?**
>
> **Конечно.** Я позволил себе немного **здорового прагматизма**, чтобы не превращать разработку в академический ад.
>
> Я не стал фанатично изгонять `ActiveRecord` из инфраструктуры или писать свои драйверы БД. Везде, где фреймворк экономит время без вреда для архитектуры (валидация форм, роутинг, работа с БД внутри репозиториев) — я его использую.
>
> **Но есть нюанс:** фреймворк заперт в клетке инфраструктуры. Он — инструмент, а не фундамент. Бизнес-логика даже не подозревает о его существовании. Подробнее об этих "сделках с совестью" читайте в разделе [Прагматизм и компромиссы](#14-прагматизм-и-компромиссы).
...
*   **Write Side (Команды):** операции изменения состояния инкапсулированы в **Use Cases** (`CreateBookUseCase`, `PublishBookUseCase`, `SubscribeUseCase`). Входные данные строго типизированы через **Command DTO** (`CreateBookCommand`).
...
Инфраструктурный слой стал еще более продвинутым в версии `0.14.0`:
*   **Multi-DB Support:** реализована полная абстракция от базы данных. Репозитории и миграции поддерживают как MySQL 8.0, так и PostgreSQL 16. Переключение драйвера происходит через ENV-переменные без изменения кода.
*   **Tracing Decorators:** для обеспечения наблюдаемости (Observability) внедрены декораторы трассировки (`QueueTracingDecorator`, `IdempotencyRepositoryTracingDecorator`). Они оборачивают вызовы инфраструктурных сервисов, добавляя spans в Inspector APM, что позволяет видеть полную картину выполнения асинхронных задач и проверок идемпотентности.
*   **PsySH:** для удобства отладки интегрирован интерактивный shell (`make shell` -> `php yii shell`), позволяющий работать с контекстом приложения в runtime.
...
*   **Решение:** используется паттерн **Fan-out**.
    1.  `PublishBookUseCase` публикует типобезопасное доменное событие `BookPublishedEvent` через метод `publishEvent()` порта `EventPublisherInterface`.
...
*   **Timeline** обработки запроса с фиксацией времени ответа.
*   **SQL & Queue Spans** — визуализация обращений к инфраструктуре (через декораторы).
*   **HTTP Context:** заголовки, Query Params и JSON Payload.
*   Детализированные сегменты обращений к БД и очередям.
