...
**Решение**: если проверка простая (формат, обязательное поле) — её делает форма. ISBN-формат проверяется кастомным `IsbnValidator`. Уникальность ISBN проверяется в Use Cases (`CreateBookUseCase`, `UpdateBookUseCase`) через checker-порт `BookIsbnCheckerInterface`; финальная защита — уникальный индекс в базе при `persist()`. Бизнес-правила (статусные переходы, валидация авторов) — в доменных сущностях.
...
## 18. Pull-модель доменных событий (RecordsEvents)

**Контекст**: доменные события (например, `BookStatusChangedEvent`) порождаются при мутации сущностей и должны публиковаться после успешного сохранения в БД.

**Проблема**: альтернатива — Use Case вызывает `eventPublisher->publish()` после каждого действия сущности. В этом случае Use Case должен знать, какие события порождает каждая операция, и дублировать эту логику. Другая альтернатива — передать `EventPublisher` в сущность через конструктор; тогда домен зависит от инфраструктуры.

**Решение**: используется **pull-модель** через трейт `RecordsEvents`. Сущность при мутации вызывает `recordEvent()`, накапливая события во внутреннем массиве. Репозиторий после `persist()` вызывает `pullRecordedEvents()`, получает накопленные события (массив очищается) и передаёт их в `TransactionalEventPublisher::publishAfterCommit()`.

**Обоснование**:

1. **Tell, Don't Ask**: сущность сама знает о своих изменениях и фиксирует их. Use Case не знает о событиях и не вызывает их — оркестрация остаётся простой.
2. **Чистота домена**: сущность не зависит от `EventPublisher` или инфраструктуры. Трейт `RecordsEvents` оперирует только массивом объектов `DomainEvent`.
3. **Один источник истины**: связь «действие → событие» живёт в сущности (например, `transitionTo()` → `BookStatusChangedEvent`), а не размазана по Use Cases.
4. **Порядок и атомарность**: репозиторий публикует события только после успешного `persist()` и в рамках той же транзакции; `publishAfterCommit` гарантирует доставку после коммита (см. пункт 1).

## 19. Сериализация событий в infrastructure (EventSerializer)

**Контекст**: контракт `DomainEvent` ранее содержал метод `getPayload(): array<string, mixed>` для сериализации события в массив примитивов. Единственный потребитель — `EventJobMappingRegistry` при создании Job из class-string маппинга (рефлексия конструктора).

**Проблема**: формат передачи в очередь — инфраструктурная ответственность. По Dependency Rule домен не должен знать, _как_ событие будет сериализовано для передачи в очередь.

**Решение**: метод `getPayload()` удалён из интерфейса `DomainEvent`. Сериализация перенесена в `EventSerializer` (infrastructure/adapters). `EventJobMappingRegistry` получает `EventSerializer` через DI и использует его при инстанцировании Job через рефлексию.

**Обоснование**: доменные события остаются чистыми typed readonly objects без знания о формате передачи. Добавление нового QueueableEvent требует только расширения `EventSerializer::serialize()` в infrastructure.
